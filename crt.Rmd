...
source("~/Dropbox/Professional/Grants_Current/SS Chemistry Data Mining/code/helper functions.R")
raw <-read.delim("~/Dropbox/Professional/Grants_Current/SS Chemistry Data Mining/data/chem_data_20170227.txt", sep = "\t")
...
#### Performance - GPA
GPA and Age have a non-linear relationship across the entire age range present. For students less than 25 years of age, the pattern of relationship between age and GPA does not significantly differ across the three course types. An interaction term between age and course type was tested in a linear model and found to be significant. This indicates that the intervention has a similar affect on a students GPA regardless of their age. 
```{r, fig.width=10}
...
```{r}
summary(aov(lm(gpa~age + crt + age*crt, data=ew)))
summary(aov(lm(gpa~age + crt, data=ew)))
```

#### Performance - DFW rate [[NOT DONE YET]]
```{r}
age.dfw.ew   <- biv.mean(df=ew, var='age', group='pass')
age.dfw.ew$plot + scale_fill_discrete(name="")
```
...
**Sumary**
* CRT has a positive effect on GPA and DWF rate after controlling for age. 
