...
```{r}
library(randomForest); library(mice)
d <- ew %>% select(-grade, -pass, -class_diff, -repeat_grade, -si.visits, -hsei_high, -origstartterm, -repeat_grade, -id, -year, -term, -sequential_term, -department_name, -major_name, -section)
...
d2 <- d2 %>% mutate(cpmath = cut(cp_math, c(0,6,8,10,17), right=FALSE),
...
rf.dta <- d2 %>% select(-DSS, -erss_enrstat, -race_code, -term_idx, -erss_totunits, 
...
* This analysis only examined courses taught by the same instructor across four semsters to assess 
the effectiveness of a single individual course redesign. 


# Next steps / Needed followup

* Enhancements to the current model
  - Controll for whether or not the student is on the second time taking the class.
