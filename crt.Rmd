...
ew$urm_status <- factor(ew$urm_status, labels=c("Non-URM", "URM", "Unknown"))
...
names(a) <- c("Repeat Grade", "N (%) Female", "N(%) Male", "p-value",  
                          "N(%) Female", "N(%) Male", "p-value")

```

```{r, results='show'}
kable(a, digits=2, align='c', caption="Comparing DFW rate across genders") %>% 
  add_header_above(c(" " = 1, "Full Sample" = 3, "F14 and F16 only" = 3)) %>%
  kable_styling(bootstrap_options="striped", full_width=F)
```
...
**Summary:** The redesign had a positive effect on GPA and DWF rate after controlling for gender. 
...
#### Course Type
The proportion of students in each URM category listed above has changed significantly since F14 ($\chi^2_{6}$=18.0, p=.006). Most notably is that the proportion of African-American and Hispanic-Latino students has increased greatly from F14 to S16, while the other groups were in decline. 
