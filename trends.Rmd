...
opts_chunk$set(warning=FALSE, message=FALSE, cache=TRUE, echo=FALSE, fig.align = 'center')
...
class <- chem %>% 
          select(classgpa, dfw_rate, enrollment, dfw, section, term_idx, year, term) %>% 
          unique()
...
### Total enrollment for Chem 111 across all sections per term
```{r}
...
Overall class sizes range from 50 to 164, with an IQR of 26, a median of 155, and a mean of 136.2.

Each dot in the figure below represents a single section. The table directly following the
figure shows the actual class size for each section that term. 

```{r}
ggplot(class, aes(x=term_idx, y=enrollment, group=term, col=term)) + 
...
ggplot(class, aes(x=term_idx, y=classgpa, group=term, col=term)) + 
  ...
  scale_color_manual(values=term.col, name = "Term") + theme_minimal() +
  scale_y_continuous(limits = c(1,3)) + ylab("Class GPA")
```
...
c_0NA <- class %>%
  filter(term_idx != "NA")
ggplot(c_0NA, aes(x=term_idx, y=dfw, group=term, col=term)) + ylab("# DFW") + 
                  geom_point() + geom_smooth(se=FALSE) + xlab("Semester") + 
                  scale_color_manual(values=term.col, name = "Term") + theme_minimal() + 
                  scale_y_continuous(limits=c(0,100), breaks=seq(0, 100, by=20)) + 
                  ggtitle("Number of students receiving a repeatable grade per term")
...
### DFW Rate
```{r}
ggplot(class, aes(x=year, y=dfw_rate, group=term, col=term)) + ylab("% DWF") + 
  geom_point() + geom_smooth(se=FALSE) + xlab("Semester") + 
  scale_color_manual(values=term.col, name = "Term")+ theme_minimal() + 
  scale_y_continuous(limits=c(0,.80), breaks=seq(0, .80, by=.20),
                     labels = scales::percent_format()) + 
  ggtitle("Percent of students receiving a repeatable grade per term") 
```
  

# Individual Student Measurements
...
samp.means <- foreach(i = 1:3, .combine = rbind) %dopar% {
                biv.mean.wide(df=chem, var=cont.varlist[i], 
