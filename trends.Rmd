...
p <- ggplot(age, aes(x=term_idx, y=mean, col=term, group=term)) +
      geom_point(size=3) + geom_smooth(se=FALSE) + geom_pointrange(aes(ymin=p10, ymax=p90))+ 
      ylab("Age") + 
      scale_color_manual(values=c("grey20", term.col), name = "Term")+ theme_bw() +
      ggtitle("Average student age per section") + xlab("Semester") + 
      scale_y_continuous(limits=c(17, 28))
  
ggplotly(p, tooltip = c("y", "x", "group")) %>% config(displayModeBar = F)
rm(ave, age, p)
```
  
The black horizonal line represents the **median** age of all students across campus -- at the time of enrollment. The vertical lines spanning the youngest 10% to the oldest 90% of ages of enrolled students during that term. From 2009 to about 2013 there was an increase in the average age of students at enrollment campus wide, which has seemed to have leveled off. 
