...
The most notable trends is the decrease in White students from being the majority at 59.6% in Fall 09 down to 44.1% by Fall 2016, with an equivelant increase in the proportion of students who identify as Hispanic/Latino increasing from 16.4% in Fall 2009 to 32.5% by Fall 2016. 
...
rct <- chem %>%
  ...
  mutate(pct=n/sum(n), term="Chem 111")
...
ggplot(art, aes(x=term_idx, y=pct, col=race_code, group=race_code)) + geom_point() +
  ...
  scale_color_discrete(name = "Race") + facet_wrap(~term)
```
...
Since the "underrepresented minority" classification tends to be a group of interest to administrators, the campus average for this group is the only one displayed on the below plot for comparisons purposes. 
...
A very similar trend is seen using the classification that lumps all students non of white or of mixed or unknown ethnicity together. Using this metric, the proportion of students of color surpasseed the proporiton of white students in Chemistry 111 by Spring 2014. 
...
### REACH
Campus level averages for the Reach program were not available. Furthermore, it appears as if the program was either not contributing data to the SIS system or not in place prior to Fall 2013. Similar to EOP, the proportion of students in the REACH program are higher in Spring compared to Fall. 
