...
#source("C:/Dropbox/Professional/Grants_Current/SS Chemistry Data Mining/code/campus_datasets.R")
rawtrends <-read.delim("C:/Dropbox/Professional/Grants_Current/SS Chemistry Data Mining/data/chem_data_20170227.txt", sep = "\t")

chem <- rawtrends %>% mutate(term_idx = factor(term_idx, 
...
a <- ggplot(pft, aes(x=term_idx, y=pf, col=term, group=term)) +
...
ggplotly(a, tooltip = c("y"))
```

 
### Age
```{r}
#ave <- filter(campus_aves, var=="age")
...
ggplot(age, aes(x=term_idx, y=mean, col=term, group=term)) +
  ...
  #geom_line(ave, aes(x=term_idx, y=mean))
